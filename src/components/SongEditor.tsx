...
import { getSortedLyricTimes, findActiveLyricIndex } from "@/lib/lyrics";
...
export function SongEditor({
  ...
  // Pre-calculate sorted line times for O(log N) lookup
  const sortedLyricTimes = useMemo(() => {
    return getSortedLyricTimes(song.lyrics);
  }, [song.lyrics]);

  // Find the active line using binary search
  const activeLineIndex = useMemo(() => {
    if (!player.isPlaying && player.currentTime === 0) return -1;
    return findActiveLyricIndex(sortedLyricTimes, player.currentTime);
  }, [player.currentTime, player.isPlaying, sortedLyricTimes]);
